...
switch ($request) {
    case 'select': //--------------------------case select ------------------------------
        $sql = "SELECT * FROM `db-fertilizer` join `dim-fertilizer` on (`dbID` = `db-fertilizer`.`FID`) WHERE `db-fertilizer`.`Name` = `dim-fertilizer`.`Name` AND `db-fertilizer`.`Alias` = `dim-fertilizer`.`Alias` AND `dim-fertilizer`.`dbID`= `db-fertilizer`.`FID`";
    ...
    case 'update': //--------------------------case update ------------------------------
        ...
        if (
            $Start == $dataSelect['Start'] && $End == $dataSelect['End'] && $Name == $dataSelect['Name'] && $Alias == $dataSelect['Alias']
    ...
    case 'insert':
        ...
        $data = [
            'FID' => $id, 'Name' => $Name, 'Alias' => $Alias, 'Unit' => $dataAll['Unit'], 'Usage' => $dataAll['Usage'], 'EQ1' => $dataAll['EQ1'],
            'EQ2' => $dataAll['EQ2'], 'StartDD' => $StartDD, 'StartMM' => $StartMM, 'EndDD' => $EndDD,
            'EndMM' => $EndMM
        ];
...
function insertLog($data)
{
    ...
    VALUES ($LOGloginID,$StartT,$StartID,$DIMfertID,$StartDD,$StartMM,$EndDD,$EndMM,$Usage,$Unit,$EQ1,$EQ2);";
