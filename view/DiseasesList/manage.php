...
switch ($request) {
  ...
  case 'insert':
    ...
    $sql = "INSERT INTO `db-pestlist` (`PID`, `Name`, `Alias`, `PTID`, `Charactor`, `Danger`, `Icon` , `NumPicChar`, `NumPicDanger`)
          VALUES ('','$Alias','$Name','2','$Charactor','$Danger','$nameImg1','$countfiles_style','$countfiles_danger')";
    //echo $sql;
    ...
    if ($countfiles_style > 0) {
      for ($i = 0; $i < $countfiles_style; $i++) {
        ...
        file_put_contents("../../picture/Pest/disease/style/$id/$nameImg2", $Pic2);
      }
    }
    ...
    if ($countfiles_danger > 0) {
      for ($i = 0; $i < $countfiles_danger; $i++) {
        ...
        file_put_contents("../../picture/Pest/disease/danger/$id/$nameImg3", $Pic3);
      }
    }
    ...
    if ($check_dim) {
      ...
      $sql = "SELECT `TypeTH` FROM `db-pesttype` WHERE `PTID` = 2";
      ...
      $sql = "INSERT INTO `dim-pest` (`ID`,`dbpestLID`,`dbpestTID`,`Name`,`Alias`,`Charactor`,`Danger`,`TypeTH`) 
                  VALUES ('','$last_id','2','$Name','$Alias','$Charactor','$Danger','$ptid')";
      echo "
      <script>
        alert($sql)
      </script>";
  ...
  case 'update':
    ...
    if ($check_dim) {
      ...
      //echo toString($ptid);
      $sql = "INSERT INTO `dim-pest` (`ID`,`dbpestLID`,`dbpestTID`,`Name`,`Alias`,`Charactor`,`Danger`,`TypeTH`) 
                  VALUES ('','$last_id','2','$nameinsect','$alias','$charstyle','$dangerInsect','$ptid')";
      echo "
        <script>
          alert($sql)
        </script>";
