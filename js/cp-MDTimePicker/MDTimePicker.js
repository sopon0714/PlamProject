/* <-----  THE DESCRIPTION DO NOT DELETE ----->
 * Add Function Format Time 24-Hr clock From Before. 
 *
 * Modify BY : CP_CPE_KU_KPS 
 * 
 * Date : 23/02/2020
 * 
 * FROM : dmuy/MDTimePicker
 * 
 * Search the function at : https://github.com/dmuy/MDTimePicker                         
 * <-----  THE DESCRIPTION DO NOT DELETE -----> */
...
function($) {
    ...
    var Time = function(hour, minute) {
            ...
            return $.trim(format.replace(/(hh|h|mm|ss|tt|t)/g, function(e) {
                switch (e.toLowerCase()) {
                    ...
                    case 'ss':
                        return '00';
                    case 't':
                        return format_24H ? '' : that.getT().toLowerCase();
                    case 'tt':
                        return format_24H ? '' : that.getT();
                }
            }));
        };
        ...
        this.getHour = function(is12Hour) {
            return is12Hour && !format_24H ? [0, 12].indexOf(this.hour) >= 0 ? 12 : (this.hour % 12) : this.hour;
        };
    ...
    var MDTimePicker = function(input, config) {
        ...
        let formatConfig = config["format"].split(":");
        format_24H = formatConfig[0] == "hh" ? true : false;

        let am_pm = format_24H ? "" : '<span class="mdtp__ampm">AM</span>';
        let am = format_24H ? "" : '<span class="mdtp__am">AM</span>';
        let pm = format_24H ? "" : '<span class="mdtp__pm">PM</span>';
        ...
        this.timepicker = {
            ...
            clockHolder: {
                ...
                buttonsHolder: {
                    ...
                    btnOk: $('<span class="mdtp__button ok">ยืนยัน</span>'),
                    btnCancel: $('<span class="mdtp__button cancel">ยกเลิก</span>'),
                }
            }
        };
        ...
        picker.clockHolder.am.click(function() {
            if (_.selected.getT() !== 'AM')
                _.setT('am');
        });
        picker.clockHolder.pm.click(function() {
            if (_.selected.getT() !== 'PM')
                _.setT('pm');
        });

        picker.timeHolder.hour.click(function() {
            if (_.activeView !== 'hours') _.switchView('hours');
        });
        picker.timeHolder.minute.click(function() {
            if (_.activeView !== 'minutes') _.switchView('minutes');
        });
    ...
    MDTimePicker.prototype = {
        ...
        setup: function(picker) {
            ...
            // Setup  1hours - 12hours
            for (var i = 0; i < 12; i++) {
                var value = i + 1,
                    deg = (HOUR_START_DEG + (i * HOUR_DEG_INCR)) % END_DEG,
                    hour = $('<div class="mdtp__digit rotate-' + deg + '" data-hour="' + value + '"><span style="color: rgb(15, 15, 15, 0.85);">' + value + '</span></div>');
            ...
            // Setup  13hours - 00hours
            if (format_24H)
                for (let j = 0; j < 12; j++, i++) {
                    var value = i + 1 == 24 ? "00" : i + 1,
                        deg = (HOUR_START_DEG + (j * HOUR_DEG_INCR)) % END_DEG,
                        hour = $('<div class="mdtp__digit rotate-' + deg + ' set24hour' + '" data-hour="' + value + '"><span>' + value + '</span></div>');
            ...
            for (var i = 0; i < 60; i++) {
                ...
                if (i % 5 === 0) minute.addClass('marker').html('<span style="color: rgb(15, 15, 15, 0.85);">' + min + '</span>');
        ...
        setHour: function(hour) {
            ...
            this.timepicker.timeHolder.hour.text(
                this.selected.getHour(true) < 10 ? '0' + this.selected.getHour(true) : this.selected.getHour(true)
            );
            this.timepicker.clockHolder.clock.hours.children('div').each(function(idx, div) {
                var el = $(div),
                    val = el.data('hour');
                let selectHours = that.selected.getHour(true) == 0 && format_24H ? "00" : that.selected.getHour(true);
                el[val === selectHours ? 'addClass' : 'removeClass']('active');
            });
        },
        ...
        setT: function(value) {
            ...
            if (this.selected.getT() !== value.toUpperCase())
                this.selected.invert();
            let t = this.selected.getT();
